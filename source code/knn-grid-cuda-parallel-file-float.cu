...
/* GPU Functions */
__global__ void nearestNeighbor( struct Element *devQ, struct Element *devC, int *devIndexArrayQ, int *devIndexArrayC, int *devNNindeces, double *devNNdistances,  int N,  int d);
__device__ double calculateDistance( struct Element a, struct Element b);
...
void readFloatsFromFile( double *a,  int N,  const char *s);

/* Validation Functions Declaration */
void validateBinning( struct Element *elemntsArray, int *a,  int N,  int d);
int validateResultsFromFile( struct Element *Q, struct Element *C, double *distances, int *NNindeces,  int N, int *oldIndexQ, int *oldIndexC);
...
int main(int argc, char  *argv[])
{
  ...
  checkCuda( cudaMalloc( &devNNdistances, N * sizeof(double)));
  ...
  checkCuda( cudaMemcpy( devNNdistances, NNdistances, N * sizeof( double), cudaMemcpyHostToDevice));
  ...
  checkCuda( cudaMemcpy(  NNdistances, devNNdistances, N * sizeof( double), cudaMemcpyDeviceToHost));
...
__global__ void nearestNeighbor( struct Element *devQ, struct Element *devC, int *devIndexArrayQ, int *devIndexArrayC, int *devNNindeces, double *devNNdistances,  int N,  int d){
  int i, k;
  int limitQ, limitC, startQ, startC, index, nearestNeighborIndex;
  ...
  for( i = index; i < limitQ; i = i + blockStride){
		minDistance = 2;
		for( k = startC; k < limitC; k++){
			distance = calculateDistance( devQ[i], devC[k]);
  ...
  int x, y, z, searchBoxId;
	double distanceZ, distanceX, distanceY, interval, blockDistance;
		      	...
		      	if( x >= 0 && x < d){
			        ...
			        if(searchBoxId != blockId_grid){
			          for( i = index; i < limitQ; i += blockStride){
			            ...
			            if( z == startZ)
			              distanceZ = abs( devQ[i].z - (z + 1)*interval);
			            else if( z == blockIdx.z)
			              distanceZ = 0;
			            else if( z == endZ)
			              distanceZ = abs( z*interval - devQ[i].z);
			            else
			              printf("Z error: z = %d\n", z);
			            /* Calculate shortest distance Of Query from Box on y axis */
			            if( y == startY)
			              distanceY = abs( devQ[i].y - (y + 1)*interval);
			            else if( y == blockIdx.y)
			              distanceY = 0;
			            else if( y == endY)
			              distanceY = abs( y*interval - devQ[i].y);
			            else
			              printf("Y error: y = %d\n", y);
			            /* Calculate shortest distance Of Query from Box on y axis */
			            if( x == startX)
			              distanceX = abs( devQ[i].x - (x + 1)*interval);
			            else if( x == blockIdx.x)
			              distanceX = 0;
			            else if( x == endX)
			              distanceX = abs( x*interval - devQ[i].x);
			            else
			              printf("X error: x = %d\n", x);
			        ...
			        __syncthreads(); /* Synchronize Threads to read simultaneously from shared memory */
			      }
			    }
			  }
			}
...
__device__
double calculateDistance( struct Element a, struct Element b)
{
	float temp;
...
int validateResultsFromFile( struct Element *Q, struct Element *C, double *distances, int *NNindeces,  int N, int *oldIndexQ, int *oldIndexC)
{
	...
	double *calculatedDistances = (double*)malloc(N*sizeof(double));
